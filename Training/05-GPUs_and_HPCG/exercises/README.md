...
4. Install HPCG
    ...
    - You need to specify the system's MPI installation path during this. Check this with the help from the advisor.
    - Include the `--gpu-aware-mpi=true` flag during installation.
    - - *Do not instruct rocHPCG to install dependencies*
    - Write down the command you used for installing HPCG.
5. Run a basic test, at max a few minutes long on just **one GPU**.
    - The executable will appear somewhere under the `build` path in HPCG.
...
6. Include the full 4 GPUs on a LUMI-G node and run a test case at max a few minutes long.
